...
// リセット用 GPIO25 を初期化＆割り込み
	...
	while(1) {
		...
		// check Memory read
		else if(control == 0x60000000) {
		    address = (bus >> 8) & 0xffff;
		    if(graphic_write_enable && address >= 0xC000) {
		        uint8_t* gvram_table[4] = {0, gvramb, gvramr, gvramg};
		        uint8_t* vram = gvram_table[graphic_write_plane];
		        data = vram ? vram[address & 0x3fff] : 0;
		        gpio_set_dir_masked(0xff,0xff);
		        gpio_put_masked(0xff,data);
